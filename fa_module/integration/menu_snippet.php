...
if (function_exists('add_menu_item')) {
    ...
    add_menu_item('Ledger', _('Sanity Check Admin'), '/modules/fa_sanity/fa_module/admin/config.php', 'SA_SANITY');
}
...
elseif (function_exists('add_menu')) {
    ...
    add_menu('Ledger', 'Sanity Check Admin', '/modules/fa_sanity/fa_module/admin/config.php', 'SA_SANITY');
}
    ...
    foreach ($menu as &$mgroup) {
        if (is_array($mgroup) && isset($mgroup['name'])) {
            ...
            if (strcasecmp($mgroup['name'], 'Ledger') === 0) {
                $mgroup['items'][] = ['label'=>_('Sanity Check Admin'), 'url'=>'/modules/fa_sanity/fa_module/admin/config.php', 'access'=>'SA_SANITY'];
            }
        }
    }
}
